 ...
 * written by Wayne Munro, //www.ros.co.nz/pdf/.  (Some performance
...
class DOMPDF {
  ...
  /**
   * Protcol used to request file (file://, //, etc)
   *
   * @var string
   */
  protected $_protocol;
  ...
  function load_html_file($file) {
    ...
    // See //the-stickman.com/web-development/php/getting-http-response-headers-when-using-file_get_contents/
  ...
  function load_html($str, $encoding = null) {
    ...
    // FIXME: roll this into the encoding detection using UTF-8/16/32 BOM (//us2.php.net/manual/en/function.mb-detect-encoding.php#91051)?
    ...
    if ( $this->get_option("enable_html5_parser") ) {
      ...
      // //devzone.zend.com/1538/php-dom-xml-extension-encoding-processing/ (see #4)
      // //stackoverflow.com/a/11310258/264628
      $doc = new DOMDocument();
    ...
    foreach($stylesheets as $tag) {
      switch (strtolower($tag->nodeName)) {
        ...
        case "style":
          ...
          // HTML 4.0 spec:
          // //www.w3.org/TR/REC-html40/present/styles.html#adef-media
