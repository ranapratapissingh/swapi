  ...
  protected function o_font($id, $action, $options = '') {
    ...
    case 'new':
      ...
      if ($this->fonts[$options['fontFileName']]['isUnicode']) {
        ...
        // transform FPDF to TCPDF (//tcpdf.sourceforge.net/)
...
//      }
  ...
  protected function o_info($id, $action, $options = '') {
    ...
    switch ($action) {
      case 'new':
        $this->infoObject = $id;
        $date = 'D:'.@date('Ymd');
        $this->objects[$id] = array('t' => 'info', 'info' => array('Creator' => 'R and OS php pdf writer, //www.ros.co.nz', 'CreationDate' => $date));
        break;
      case 'Title':
      case 'Author':
      case 'Subject':
   ...
   * RFC for UTF-8 at //www.faqs.org/rfcs/rfc3629.html
   *
   * @access private
  ...
  function utf8toCodePointsArray(&$text) {
    $length = mb_strlen($text, '8bit'); // //www.php.net/manual/en/function.mb-strlen.php#77040
